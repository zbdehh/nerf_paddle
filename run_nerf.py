...
def batchify(fn, chunk):
    ...
    def ret(inputs):
        return paddle.concat([fn(inputs[i:i + chunk]) for i in range(0, inputs.shape[0], chunk)], 0)
...
def batchify_rays(rays_flat, chunk=1024 * 32, **kwargs):
    ...
    all_ret = {k: paddle.concat(all_ret[k], 0) for k in all_ret}
    return all_ret
...
def render(H, W, K, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
    ...
    if use_viewdirs:
        ...
        viewdirs = paddle.reshape(viewdirs, [-1, 3])
    ...
    # Create ray batch
    rays_o = paddle.reshape(rays_o, [-1, 3])
    rays_d = paddle.reshape(rays_d, [-1, 3])

    near, far = near * paddle.ones_like(rays_d[..., :1]), far * paddle.ones_like(rays_d[..., :1])
    rays = paddle.concat([rays_o, rays_d, near, far], -1)
    if use_viewdirs:
        rays = paddle.concat([rays, viewdirs], -1)
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    weights = alpha * paddle.cumprod(paddle.concat([paddle.ones((alpha.shape[0], 1)), 1. - alpha + 1e-10], -1), -1)[:,
                      :-1]
...
def render_rays(ray_batch,
    ...
    if perturb > 0.:
        ...
        upper = paddle.concat([mids, z_vals[..., -1:]], -1)
        lower = paddle.concat([z_vals[..., :1], mids], -1)
    ...
    if N_importance > 0:
        ...
        z_vals= paddle.sort(paddle.concat([z_vals, z_samples], -1), -1).astype('float32')
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            batch = paddle.transpose(batch, [1, 0, 2])
            ...
            if N_rand is not None:
                ...
                if i < args.precrop_iters:
                    ...
                    coords = paddle.stack(paddle.meshgrid(paddle.linspace(0, H - 1, H), paddle.linspace(0, W - 1, W)),
                                          -1)  # (H, W, 2)
                ...
                batch_rays = paddle.stack([rays_o, rays_d], 0)
                target_s = target[select_coords[:, 0], select_coords[:, 1]].astype('float32')  # (N_rand, 3)
        ...
        if i % args.i_testset == 0 and i > 0:
            ...
            with paddle.no_grad():
                render_path(paddle.to_tensor(poses[i_test]), hwf, K, args.chunk, render_kwargs_test,
