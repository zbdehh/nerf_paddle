...
class Embedder:
    ...
    def embed(self, inputs):
        return paddle.concat([fn(inputs) for fn in self.embed_fns], -1)
...
class NeRF(nn.Layer):
    ...
    def forward(self, x):
        input_pts, input_views = paddle.split(x, [self.input_ch, self.input_ch_views], axis=-1)
        h = input_pts
        for i, l in enumerate(self.pts_linears):
            h = self.pts_linears[i](h)
            h = F.relu(h)
            if i in self.skips:
                h = paddle.concat([input_pts, h], -1)

        if self.use_viewdirs:
            alpha = self.alpha_linear(h)
            feature = self.feature_linear(h)
            h = paddle.concat([feature, input_views], -1)
...
def get_rays(H, W, K, c2w):
    ...
    i = i.t()
    j = j.t()
    dirs = paddle.stack([(i - K[0][2]) / K[0][0], -(j - K[1][2]) / K[1][1], -paddle.ones_like(i)], -1)
...
def ndc_rays(H, W, focal, near, rays_o, rays_d):
    ...
    rays_o = paddle.stack([o0, o1, o2], -1)
    rays_d = paddle.stack([d0, d1, d2], -1)
...
def sample_pdf(bins, weights, N_samples, det=False, pytest=False):
    ...
    pdf = weights / paddle.sum(weights, -1, keepdim=True)
    cdf = paddle.cumsum(pdf, -1)
    cdf = paddle.concat([paddle.zeros_like(cdf[..., :1]), cdf], -1)  # (batch, len(bins))
    ...
    inds_g = paddle.stack([below, above], -1)  # (batch, N_samples, 2)
